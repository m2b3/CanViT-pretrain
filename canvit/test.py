...
def test_canvit_init_canvas():
    ...
    cfg = CanViTConfig(n_canvas_registers=8, canvas_num_heads=8, canvas_head_dim=8)
...
def test_canvit_forward():
    ...
    cfg = CanViTConfig(n_canvas_registers=8, adapter_stride=2, layer_scale_init=1e-3, canvas_num_heads=8, canvas_head_dim=8)
    model = CanViT(backbone, cfg)
...
def test_canvit_n_adapters():
    ...
    cfg = CanViTConfig(adapter_stride=2, canvas_num_heads=8, canvas_head_dim=8)
    model = CanViT(backbone, cfg)
    assert model.n_adapters == 5  # (12-1) // 2 = 5

    cfg3 = CanViTConfig(adapter_stride=3, canvas_num_heads=8, canvas_head_dim=8)
...
def test_canvit_gradients_flow():
    ...
    cfg = CanViTConfig(n_canvas_registers=4, adapter_stride=2, canvas_num_heads=8, canvas_head_dim=8)
    model = CanViT(backbone, cfg)
